...
2. **标准工作流**
   ...
   - **阶段三：执行修改 (Modify)**
	 ...
	 - **属性查看**：
	   - 调用 `check_node_properties` (node_path="目标节点路径") 确认属性名和类型。
	 - **属性修改**：
...
### 拓展知识
如需深入了解不同类型场景的搭建规范，请使用相应的工具查阅 `res://addons/godot_ai_chat/skills/scene_tool_sop/reference/` 文件夹中的引用文件。
...
**修改属性流程：**
1. 用户："把 Sprite 变红"
2. AI 调用 `check_node_properties(node_path="Sprite")`
