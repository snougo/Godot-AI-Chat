...
func create_new_chat() -> void:
	var history: ChatMessageHistory = _session_manager.create_new_session()
...
func fork_current_chat() -> void:
	...
	var saved_history: ChatMessageHistory = _session_manager.fork_session(forked_history)
	if saved_history:
...
func load_chat(p_session_name: String) -> void:
	var history: ChatMessageHistory = _session_manager.load_session(p_session_name)
...
func delete_chat(p_session_name: String) -> void:
	var is_current: bool = (_session_manager.current_session_path.get_file() == p_session_name)
	...
	if is_current:
		var loaded_history: ChatMessageHistory = _session_manager.load_latest_session()
		if loaded_history:
			var loaded_name: String = _session_manager.current_session_path.get_file()
...
func create_session_from_history(p_history: ChatMessageHistory) -> bool:
	var saved_history: ChatMessageHistory = _session_manager.create_session_from_history(p_history)
...
func handle_workspace_change(p_new_path: String) -> void:
	...
	var plugin_settings_res: PluginSettingsConfig = ToolBox.get_plugin_settings()
...
func load_latest_on_init() -> void:
	if not _session_manager.has_active_session():
		var history: ChatMessageHistory = _session_manager.load_latest_session()
		if history:
			_load_history_to_ui(history, _session_manager.current_session_path.get_file())
...
# 将历史加载到 UI 并绑定轮数更新信号
func _load_history_to_ui(p_history: ChatMessageHistory, p_filename: String) -> void:
	_chat_ui.select_session_by_name(p_filename)
	# 用存档中的累计用量恢复显示；新建/分叉会话若为全零，效果等同重置
	_chat_ui.restore_token_usage(p_history.token_usage)
	_current_chat_window.load_session_history_resource(p_history)
	
	if p_history.changed.is_connected(update_turn_info):
		p_history.changed.disconnect(update_turn_info)
	p_history.changed.connect(update_turn_info)
