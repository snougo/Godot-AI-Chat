...
## 执行文件重命名操作
## [param p_args]: 包含 source_path 和 new_name 的参数字典
...
func execute(p_args: Dictionary) -> Dictionary:
	...
	if source_path.is_empty() or new_name.is_empty():
		return {"success": false, "data": "Error: Both source_path and new_name are required."}
	...
	if DirAccess.dir_exists_absolute(source_path):
		return {"success": false, "data": "Error: '%s' is a directory. Cannot rename directories." % source_path}
	...
	if "/" in new_name or "\\" in new_name:
		return {"success": false, "data": "Error: new_name must be a filename only, without any path components."}
	
	# 校验 new_name：不能为空
	if new_name.is_empty():
		return {"success": false, "data": "Error: new_name cannot be empty."}
	
	# 校验 new_name：必须与原名不同
	var current_filename: String = source_path.get_file()
	if new_name == current_filename:
		return {"success": false, "data": "Error: new_name is identical to the current filename '%s'." % current_filename}
	...
	if err != OK:
		return {"success": false, "data": "Failed to rename. Error code: " + str(err)}
	
	ToolBox.refresh_editor_filesystem()
	return {"success": true, "data": "Successfully renamed '%s' to '%s'." % [source_path, target_path]}
