...
static func mount_skill(_skill_name: String) -> bool:
	if not available_skills.has(_skill_name):
		push_error("[ToolRegistry] Cannot mount unknown skill: %s" % _skill_name)
		return false
	
	if _skill_name in active_skills_list:
		print("[ToolRegistry] Skill '%s' is already mounted." % _skill_name)
		return true
	
	# 添加到列表末尾 (优先级最高，覆盖前面的)
	active_skills_list.append(_skill_name)
	print("[ToolRegistry] Mounting skill: %s" % _skill_name)
...
## 卸载一个技能
static func unmount_skill(_skill_name: String) -> void:
	if not _skill_name in active_skills_list:
		return
	
	active_skills_list.erase(_skill_name)
	print("[ToolRegistry] Unmounting skill: %s" % _skill_name)
	
	rebuild_tool_set()


## 检查某个技能是否已激活
static func is_skill_active(_skill_name: String) -> bool:
	return _skill_name in active_skills_list
...
static func rebuild_tool_set() -> void:
	...
	for skill_name in active_skills_list:
		...
		var skill: Object = available_skills[skill_name]
		#print("skill type: ", typeof(skill))
		if "tools" in skill:
	...
	for skill_name in active_skills_list:
		...
		if "instruction_file" in skill and not skill.instruction_file.is_empty():
			if FileAccess.file_exists(skill.instruction_file):
				var content: String = FileAccess.get_file_as_string(skill.instruction_file)
...
static func get_tool(_tool_name: String) -> Object:
	# 安全检查：防止脚本重载后状态丢失
...
static func _load_skill_from_folder(_folder_path: String) -> void:
	...
	if dir:
		...
		while file_name != "":
			if not dir.current_is_dir() and (file_name.ends_with(".tres") or file_name.ends_with(".res")):
				var resource: Object = load(_folder_path.path_join(file_name))
				#print("resource type: ", typeof(resource))
				# 详细判断并打印结果
				if resource:
...
static func _load_and_register_tool(_path: String) -> void:
	if not FileAccess.file_exists(_path): return
	var script: Object = load(_path)
	#print("script type: ", typeof(script))
	if script:
