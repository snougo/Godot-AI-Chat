...
# 统一的文件扩展名检查函数
func validate_file_extension(_path: String, _allowed_extensions: Array = []) -> String:
	if _allowed_extensions.is_empty():
		_allowed_extensions = DEFAULT_ALLOWED_EXTENSIONS
	
	var extension: String = _path.get_extension().to_lower()
	if extension not in _allowed_extensions:
		return "Error: File extension '%s' is not allowed. Allowed extensions: %s" % [extension, str(_allowed_extensions)]
...
func _parse_script_to_slices(_code: String) -> Array:
	var lines: PackedStringArray = _code.split("\n")
	var slices := []
	var current_start := 0
	
	for i in range(lines.size()):
		var line = lines[i]
		print("line type:", typeof(line))
...
func _find_code_in_slice(_editor: Control, _slice: Dictionary, _code_to_find: String) -> Dictionary:
	var slice_start = _slice.start_line
	var slice_end = _slice.end_line
	print("slice_start: ", typeof(slice_start))
	print("slice_end: ", typeof(slice_end))
	var find_lines := []
	for line in _code_to_find.split("\n"):
	...
	for i in range(slice_start, slice_end + 1):
		...
		while match_cursor < find_lines.size() and current_file_line <= slice_end:
			var file_line_content = _editor.get_line(current_file_line).strip_edges()
...
# 获取上下文预览
func _get_preview_lines(_editor: Control, _slice: Dictionary, _count: int) -> String:
	var txt := ""
	var limit := min(_slice.end_line, _slice.start_line + _count)
	for i in range(_slice.start_line, limit + 1):
		txt += _editor.get_line(i) + "\n"
	return txt


# 获取一行的缩进字符串（Tab 或空格）
func _get_indentation(_line: String) -> String:
	var indent := ""
	for char in _line:
		if char == " " or char == "\t":
			indent += char
...
# 智能应用缩进到多行代码
func _apply_indentation(_code: String, _indent: String) -> String:
	if _indent.is_empty():
		return _code
