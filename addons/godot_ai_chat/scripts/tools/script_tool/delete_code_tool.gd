...
func execute(p_args: Dictionary) -> ToolResult:
	...
	if start_line < 1 or end_line < 1 or start_line > total_lines or end_line > total_lines:
		return ToolResult.fail("Line numbers out of bounds. Valid range: 1-%d, got start_line=%d, end_line=%d" % [total_lines, start_line, end_line])
	
	if start_line > end_line:
		return ToolResult.fail("Invalid range: start_line (%d) cannot be greater than end_line (%d)." % [start_line, end_line])
	...
	return ToolResult.ok("Deleted lines %d-%d.\n\nCurrent Script:\n%s" % [start_line, end_line, view])
