...
func _parse_keywords(_keyword_input: Variant) -> PackedStringArray:
	var result: PackedStringArray = []
	
	if _keyword_input is Array:
		for i in _keyword_input:
			result.append(str(i).strip_edges())
	else:
		var split_keywords: String = str(_keyword_input)
	...
	if not fuzzy_matches.is_empty():
		...
		if exact_matches.is_empty():
			...
			if fuzzy_matches.size() > max_fuzzy_details:
				var rest: Array[String] = fuzzy_matches.slice(max_fuzzy_details)
			...
			# Just list names
			var display_list: Array[String] = fuzzy_matches
