...
func execute(p_args: Dictionary) -> ToolResult:
	...
	if not viewport:
		return ToolResult.fail("No 2D viewport available.")
	...
	if p_args.has("zoom") and not p_args["zoom"] == null:
		...
		if zoom_val <= 0:
			return ToolResult.fail("Zoom must be positive, got %f." % zoom_val)
	...
	if p_args.has("pan_offset") and not p_args["pan_offset"] == null:
		...
		if pan.size() < 2:
			return ToolResult.fail("pan_offset requires 2 values [x, y], got %d." % pan.size())
