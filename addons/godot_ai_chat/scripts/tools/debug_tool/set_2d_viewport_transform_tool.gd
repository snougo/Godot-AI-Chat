...
func execute(p_args: Dictionary) -> Dictionary:
	...
	if p_args.has("zoom") and not p_args["zoom"] == null:
		...
		if zoom_val <= 0:
			return {"success": false, "data": "Zoom must be positive, got %f." % zoom_val}
	...
	if p_args.has("pan_offset") and not p_args["pan_offset"] == null:
		...
		if pan.size() < 2:
			return {"success": false, "data": "pan_offset requires 2 values [x, y], got %d." % pan.size()}
		xform.origin = Vector2(float(pan[0]), float(pan[1]))
		changes.append("pan → (%.0f, %.0f)" % [float(pan[0]), float(pan[1])])
	
	if changes.is_empty():
		var current_scale: Vector2 = xform.get_scale()
		return {"success": true, "data": "No parameters provided. Transform unchanged. Current zoom: (%.2fx, %.2fx), pan: %s" % [current_scale.x, current_scale.y, str(xform.origin)]}
	
	viewport.global_canvas_transform = xform
	return {"success": true, "data": "2D viewport transform updated: %s" % ", ".join(changes)}
