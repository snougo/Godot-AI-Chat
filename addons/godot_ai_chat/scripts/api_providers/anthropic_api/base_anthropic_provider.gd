@tool
class_name BaseAnthropicProvider
extends BaseLLMProvider

## Anthropic 协议基类
##
## 处理 Claude 系列模型的 Message API 格式构建和 SSE 流式响应解析。


# --- Constants ---

## 第三方兼容服务（如 DeepSeek Anthropic 端点）可能不支持 system 数组 + cache_control 格式
## 若收到 400 报错，改为 false 回退为字符串 system
const USE_SYSTEM_CACHE_CONTROL := true
## 同理，部分网关不接受 tools 内的 cache_control
const USE_TOOLS_CACHE_CONTROL := true


# --- Private Vars ---

## 本轮流内累计的用量（Anthropic 把 input/output 拆到不同事件下发，需自行合并）
var _current_stream_usage: Dictionary = {}


# --- Public Functions ---

## 重置流内状态
func reset_stream_state() -> void:
	_current_stream_usage.clear()


func get_stream_parser_type() -> StreamParserType:
	return StreamParserType.SSE


## [子类覆写] 构建请求体 (Body)
func build_request_body_impl(p_model_name: String, p_messages: Array[ChatMessage], p_temperature: float, p_stream: bool, p_tool_definitions: Array = []) -> Dictionary:
	# kimi-k2.6 模型只接受固定温度值 1，强制覆盖用户设置
	var final_temperature: float = 1.0 if p_model_name == "kimi-k2.6" else p_temperature
	
	var system_prompt: String = ""
	var api_messages: Array[Dictionary] = []
	
	for msg: ChatMessage in p_messages:
		if msg.role == ChatMessage.ROLE_SYSTEM:
			if not system_prompt.is_empty():
				system_prompt += "\n"
			system_prompt += msg.content
		else:
			api_messages.append(_convert_message_to_anthropic(msg))
	
	# 统一规范化处理：合并连续角色、清理孤立工具、移除空消息（遵循Anthropic官方的API端点设计标准）
	# 注意硅基流动等API提供商并不真正支持Anthropic API端点，其服务器内部实现是将openAI API映射成Anthropic API
	# 如果你一定要在Anthropic API下使用硅基流动的API，请将下面这条代码注释掉
	# 对于其他遵循Anthropic API端点标准的服务商，则不需要这么做
	api_messages = _normalize_anthropic_messages(api_messages)
	
	var body: Dictionary = {
		"model": p_model_name,
		"messages": api_messages,
		"temperature": snappedf(final_temperature, 0.1),
		"max_tokens": _resolve_max_tokens(p_model_name),
		"stream": p_stream
	}
	
	if not system_prompt.is_empty():
		if USE_SYSTEM_CACHE_CONTROL:
			# 使用数组格式并标记 cache_control，启用 Anthropic Prompt Caching
			body["system"] = [
				{"type": "text", "text": system_prompt, "cache_control": {"type": "ephemeral"}}
			]
		else:
			body["system"] = system_prompt
	
	# 工具定义：LLM API 无状态，每个请求都必须携带完整 tools 定义
	# （包括工具结果回传后的后续请求，否则模型会退化为文本格式模拟工具调用）
	if not p_tool_definitions.is_empty():
		var anthropic_tools: Array = []
		for raw_tool: Variant in p_tool_definitions:
			if not raw_tool is Dictionary:
				continue
			var tool: Dictionary = _convert_tool_definition(raw_tool)
			if USE_TOOLS_CACHE_CONTROL:
				tool["cache_control"] = {"type": "ephemeral"}
			anthropic_tools.append(tool)
		body["tools"] = anthropic_tools
	
	return body


## 解析非流式响应 (完整 Body)
func parse_non_stream_response(p_body_bytes: PackedByteArray) -> Dictionary:
	var json_str: String = p_body_bytes.get_string_from_utf8()
	var json: Variant = JSON.parse_string(json_str)
	
	if json == null:
		AIChatLogger.error("BaseAnthropicProvider: Failed to parse JSON response")
		return {"error": "Invalid JSON response", "raw": json_str.substr(0, 500)}
	
	if not json is Dictionary:
		AIChatLogger.error("BaseAnthropicProvider: Response is not a Dictionary")
		return {"error": "Unexpected response format", "raw": str(json).substr(0, 500)}
	
	var body: Dictionary = json
	
	if body.has("error"):
		var error_msg: String = str(body.error)
		if body.error is Dictionary:
			error_msg = String(body.error.get("message", str(body.error)))
		AIChatLogger.error("BaseAnthropicProvider: API Error: " + error_msg)
		return {"error": error_msg, "raw": body}
	
	if body.has("content"):
		var result: Dictionary = {
			"role": body.get("role", "assistant"),
			"content": "",
			"tool_calls": []
		}
		
		if body.content is Array:
			for raw_block: Variant in (body.content as Array):
				if not raw_block is Dictionary:
					continue
				var block: Dictionary = raw_block
				
				var block_type: String = String(block.get("type", ""))
				if block_type == "text":
					result.content += String(block.get("text", ""))
				elif block_type == "tool_use":
					var input_data: Variant = block.get("input", {})
					var input_str: String = str(input_data)
					if input_data is Dictionary or input_data is Array:
						input_str = JSON.stringify(input_data)
					result.tool_calls.append({
						"id": block.get("id", ""),
						"type": "function",
						"function": { "name": block.get("name", ""), "arguments": input_str }
					})
		
		if body.has("usage") and body.usage is Dictionary:
			result.usage = _normalize_usage(body.usage)
		return result
	
	return {"error": "Unknown response format", "raw": json_str.substr(0, 500)}


## 解析单个流式数据块为协议无关增量
## Anthropic 的 content block 索引在单轮流内稳定且互异，直接作为工具槽位键
func parse_stream_chunk(p_raw_chunk: Dictionary) -> LLMStreamDelta:
	var result: LLMStreamDelta = LLMStreamDelta.new()
	var event_type: String = String(p_raw_chunk.get("type", ""))
	var block_index: int = int(p_raw_chunk.get("index", -1))
	
	match event_type:
		"message_start":
			_current_stream_usage.clear()
			var raw_message: Variant = p_raw_chunk.get("message")
			if raw_message is Dictionary:
				var raw_usage: Variant = (raw_message as Dictionary).get("usage")
				if raw_usage is Dictionary:
					_update_and_normalize_usage(raw_usage)
					result.usage = _current_stream_usage.duplicate()
		
		"content_block_start":
			var block: Dictionary = {}
			var raw_block: Variant = p_raw_chunk.get("content_block")
			if raw_block is Dictionary:
				block = raw_block
			
			var block_type: String = String(block.get("type", ""))
			if block_type == "tool_use":
				result.tool_call_deltas.append(LLMStreamDelta.make_tool_call_start(
					str(block_index),
					String(block.get("id", "")),
					String(block.get("name", ""))
				))
			elif block_type == "thinking":
				# 捕获 thinking 块签名（extended thinking 多轮回传必需，缺失会报 400）
				var signature: String = String(block.get("signature", ""))
				if not signature.is_empty():
					result.metadata_updates[ChatMessage.META_THINKING_SIGNATURE] = signature
		
		"content_block_delta":
			var delta: Dictionary = {}
			var raw_delta: Variant = p_raw_chunk.get("delta")
			if raw_delta is Dictionary:
				delta = raw_delta
			
			var delta_type: String = String(delta.get("type", ""))
			match delta_type:
				"text_delta":
					result.content_delta = String(delta.get("text", ""))
				"thinking_delta":
					result.reasoning_delta = String(delta.get("thinking", ""))
				"input_json_delta":
					var fragment: String = String(delta.get("partial_json", delta.get("args", delta.get("arguments", ""))))
					if not fragment.is_empty():
						result.tool_call_deltas.append(
							LLMStreamDelta.make_tool_call_arguments_append(str(block_index), fragment)
						)
		
		"message_delta":
			var raw_usage: Variant = p_raw_chunk.get("usage")
			if raw_usage is Dictionary:
				_update_and_normalize_usage(raw_usage)
				result.usage = _current_stream_usage.duplicate()
		
		"message_stop":
			result.is_stream_finished = true
	
	return result


# --- Private Functions ---

# 基于状态收敛的 Anthropic 消息规范化器
# 自动合并连续角色、清除孤立工具调用、清除空消息
func _normalize_anthropic_messages(p_messages: Array[Dictionary]) -> Array[Dictionary]:
	var result: Array[Dictionary] = p_messages.duplicate(true)
	var changed: bool = true
	
	# 如果数组发生任何合并或删除，触发重新检查，直到结构绝对稳定且完美符合要求
	while changed:
		changed = false
		
		# 1. 合并连续相同角色的消息
		var merged: Array[Dictionary] = []
		for msg: Dictionary in result:
			if merged.is_empty():
				merged.append(msg)
				continue
			
			var last_msg: Dictionary = merged.back()
			if last_msg.role == msg.role:
				changed = true
				var c1: Array = _to_content_blocks(last_msg.content)
				var c2: Array = _to_content_blocks(msg.content)
				last_msg.content = c1 + c2
				if msg.role == "user":
					# Anthropic 规则：tool_result 块必须位于 user 消息所有文本块之前
					# 重排合并结果：tool_result 全部前置（保持相对顺序），其余块保持原顺序
					var tool_results: Array = []
					var others: Array = []
					for b: Variant in (last_msg.content as Array):
						if b is Dictionary and b.get("type") == "tool_result":
							tool_results.append(b)
						else:
							others.append(b)
					last_msg.content = tool_results + others
			else:
				merged.append(msg)
		
		result = merged
		
		# 2. 清理孤立的 tool_use 和 tool_result
		for i in range(result.size()):
			var msg: Dictionary = result[i]
			if not msg.content is Array:
				continue
			
			if msg.role == "assistant":
				var valid_ids: Dictionary = {}
				if i + 1 < result.size() and result[i + 1].role == "user" and result[i + 1].content is Array:
					for b: Variant in (result[i + 1].content as Array):
						if b is Dictionary and b.get("type") == "tool_result":
							valid_ids[b.get("tool_use_id")] = true
				
				var new_content: Array = []
				for b: Variant in (msg.content as Array):
					if b is Dictionary and b.get("type") == "tool_use":
						if valid_ids.has(b.get("id")):
							new_content.append(b)
						else:
							changed = true
							AIChatLogger.warn("[BaseAnthropic] Removed orphaned tool_use: " + str(b.get("id", "")))
					else:
						new_content.append(b)
				msg.content = new_content
			
			elif msg.role == "user":
				var valid_ids: Dictionary = {}
				if i - 1 >= 0 and result[i - 1].role == "assistant" and result[i - 1].content is Array:
					for b: Variant in (result[i - 1].content as Array):
						if b is Dictionary and b.get("type") == "tool_use":
							valid_ids[b.get("id")] = true
				
				var new_content: Array = []
				for b: Variant in (msg.content as Array):
					if b is Dictionary and b.get("type") == "tool_result":
						if valid_ids.has(b.get("tool_use_id")):
							new_content.append(b)
						else:
							changed = true
							AIChatLogger.warn("[BaseAnthropic] Removed orphaned tool_result: " + str(b.get("tool_use_id", "")))
					else:
						new_content.append(b)
				msg.content = new_content
		
		# 3. 移除空消息
		var no_empty: Array[Dictionary] = []
		for msg: Dictionary in result:
			if msg.content is Array:
				# 过滤数组内的空文本块（Anthropic 禁止空 text 块，会报 400）
				var filtered: Array = []
				for b: Variant in (msg.content as Array):
					if b is Dictionary and b.get("type") == "text" and String(b.get("text", "")).is_empty():
						changed = true
						continue
					filtered.append(b)
				msg.content = filtered
				
				if msg.content.is_empty():
					changed = true
					continue
				if msg.content.size() == 1 and msg.content[0].get("type") == "text":
					msg.content = msg.content[0].get("text", "") # 简化回纯文本
			elif msg.content is String and msg.content.is_empty():
				changed = true
				continue
			no_empty.append(msg)
		
		result = no_empty
	
	return result


# 合并并归一化本轮流内用量（input/output 分事件下发）
func _update_and_normalize_usage(p_new_usage: Dictionary) -> void:
	if p_new_usage.has("input_tokens"):
		_current_stream_usage["prompt_tokens"] = int(p_new_usage["input_tokens"])
	if p_new_usage.has("output_tokens"):
		_current_stream_usage["completion_tokens"] = int(p_new_usage["output_tokens"])
	if _current_stream_usage.has("prompt_tokens") and _current_stream_usage.has("completion_tokens"):
		_current_stream_usage["total_tokens"] = int(_current_stream_usage["prompt_tokens"]) + int(_current_stream_usage["completion_tokens"])


# 归一化非流式用量
func _normalize_usage(p_usage: Dictionary) -> Dictionary:
	var result: Dictionary = {}
	if p_usage.has("input_tokens"):
		result["prompt_tokens"] = int(p_usage["input_tokens"])
	if p_usage.has("output_tokens"):
		result["completion_tokens"] = int(p_usage["output_tokens"])
	if result.has("prompt_tokens") and result.has("completion_tokens"):
		result["total_tokens"] = int(result["prompt_tokens"]) + int(result["completion_tokens"])
	return result


# 将消息 content 统一转为内容块数组（字符串转 text 块，数组原样返回）
func _to_content_blocks(p_content: Variant) -> Array:
	if p_content is Array:
		return p_content
	return [{"type": "text", "text": p_content}]


func _convert_tool_definition(p_openai_tool: Dictionary) -> Dictionary:
	if p_openai_tool.has("input_schema"):
		return p_openai_tool
	if p_openai_tool.has("function"):
		var func_def: Dictionary = p_openai_tool.function
		var tool: Dictionary = { "name": func_def.get("name", ""), "input_schema": func_def.get("parameters", {}) }
		if func_def.has("description"):
			tool.description = func_def.description
		return tool
	return p_openai_tool


func _convert_message_to_anthropic(p_msg: ChatMessage) -> Dictionary:
	var role: String = p_msg.role
	var content: Variant = p_msg.content
	
	if role == ChatMessage.ROLE_TOOL:
		role = "user"
		content = [{ "type": "tool_result", "tool_use_id": p_msg.tool_call_id, "content": p_msg.content }]
	elif role == ChatMessage.ROLE_USER:
		content = _build_user_content(p_msg)
	elif role == ChatMessage.ROLE_ASSISTANT:
		content = _build_assistant_content(p_msg)
	
	return { "role": role, "content": content }


func _build_user_content(p_msg: ChatMessage) -> Variant:
	var has_images: bool = false
	if not p_msg.images.is_empty():
		for img: Dictionary in p_msg.images:
			if img.has("data") and img.data is PackedByteArray:
				has_images = true
				break
	
	if not has_images:
		return p_msg.content
	
	var content_array: Array = []
	for img: Dictionary in p_msg.images:
		if not img.has("data") or not img.data is PackedByteArray:
			continue
		
		var media_type: String = img.get("mime", "image/jpeg")
		content_array.append({
			"type": "image",
			"source": { "type": "base64", "media_type": media_type, "data": Marshalls.raw_to_base64(img.data) }
		})
	
	if not p_msg.content.is_empty():
		content_array.append({ "type": "text", "text": p_msg.content })
	
	return content_array


func _build_assistant_content(p_msg: ChatMessage) -> Variant:
	if p_msg.tool_calls.is_empty() and p_msg.reasoning_content.is_empty():
		return p_msg.content
	
	var content_array: Array = []
	if not p_msg.reasoning_content.is_empty():
		var thinking_block: Dictionary = { "type": "thinking", "thinking": p_msg.reasoning_content }
		var thinking_signature: String = String(p_msg.metadata.get(ChatMessage.META_THINKING_SIGNATURE, ""))
		if not thinking_signature.is_empty():
			thinking_block.signature = thinking_signature
		content_array.append(thinking_block)
	
	if not p_msg.content.is_empty():
		content_array.append({ "type": "text", "text": p_msg.content })
	
	for raw_call: Variant in p_msg.tool_calls:
		if not raw_call is Dictionary:
			continue
		
		var tc: Dictionary = raw_call
		var args_obj: Dictionary = {}
		var raw_function: Variant = tc.get("function")
		
		if raw_function is Dictionary:
			var args_str: String = String((raw_function as Dictionary).get("arguments", ""))
			if not args_str.is_empty():
				var parsed: Variant = JSON.parse_string(args_str)
				if parsed is Dictionary or parsed is Array:
					args_obj = parsed
				else:
					AIChatLogger.warn("[BaseAnthropicProvider] Invalid tool arguments JSON: " + args_str.substr(0, 200))
		
		var raw_func: Variant = tc.get("function", {})
		var func_dict: Dictionary = raw_func if raw_func is Dictionary else {}
		content_array.append({
			"type": "tool_use",
			"id": tc.get("id", ""),
			"name": func_dict.get("name", ""),
			"input": args_obj
		})
	
	return content_array


# 按模型解析 max_tokens（官方 API 中该字段必填，且不能超过模型上限）
func _resolve_max_tokens(p_model_name: String) -> int:
	# 旧版 haiku 上限较低，其余现代模型均支持 8192+
	if p_model_name.begins_with("claude-3-haiku"):
		return 4096
	return 8192
